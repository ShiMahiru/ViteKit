...
const postModules = import.meta.glob('/src/posts/*/index.md', { eager: true });
const rawPostModules = import.meta.glob('/src/posts/*/index.md', {
	eager: true,
	query: '?raw',
	import: 'default'
});
...
export async function getPostComponent(slug: string) {
	try {
		const modules = import.meta.glob('/src/posts/*/index.md');
		const path = `/src/posts/${slug}/index.md`;
