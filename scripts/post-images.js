...
async function main() {
  ...
  if (!fs.existsSync(postsDir)) {
    console.log('[post-images] src/posts 不存在，跳过');
    return;
  }
  ...
  const postDirs = fs.readdirSync(postsDir).filter((name) => {
    const postDir = path.join(postsDir, name);
    return fs.statSync(postDir).isDirectory() && fs.existsSync(path.join(postDir, 'index.md'));
  });
